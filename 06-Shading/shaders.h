...
namespace ShaderProgram
{
  enum
  {
    Default, Billboarding, PointRendering, Tonemapping, NumShaderPrograms,
  };
}
...
namespace VertexShader
{
  enum
  {
    Default, Billboarding, Point, ScreenQuad, NumVertexShaders
  };
}
...
// ----------------------------------------------------------------------------
// Billboard vertex shader using instancing buffer via uniform block objects
// ----------------------------------------------------------------------------
R"(
#version 330 core
...
layout (binding = 1) uniform InstanceBuffer
{
  ...
  InstanceData instanceBuffer[512];
};
...
{
  // Pass texture coordinates to the fragment shader
  vec2 steps = vec2(instanceBuffer[gl_InstanceID].time.xy);
  float newX = texCoord.s/8 +steps.x/8;
  float newY = 1-(texCoord.t/8 +steps.y/8);
  vOut.texCoord = vec2(newX,newY);
  ...
  rotation[1] = vec3(0,0,PI);
  rotation[2] = vec3(0,-PI,0);

  
  vOut.worldPos = vec4(vec4(position.xyz*rotation, 1.0f) * modelToWorld, 1.0f);
  mat3x4 view = worldToView;
  view[0][0] = 1.0f;
  view[0][1] = 0.0f;
  view[0][2] = 0.0f;
  view[1][0] = 0.0f;
  ...
  vec4 viewPos = vec4(vOut.worldPos * view, 1.0f);

  vec4 screenspace = projection * viewPos;
...
void main()
{
  ...
  color = vec4(finalColor, texture(Diffuse, vIn.texCoord.st).a);
}
)",
// ----------------------------------------------------------------------------
