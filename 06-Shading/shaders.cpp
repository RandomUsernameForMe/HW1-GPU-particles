...
bool compileShaders()
{
  ...
  shaderProgram[ShaderProgram::Billboarding] = glCreateProgram();
  glAttachShader(shaderProgram[ShaderProgram::Billboarding], vertexShader[VertexShader::Billboarding]);
  glAttachShader(shaderProgram[ShaderProgram::Billboarding], fragmentShader[FragmentShader::Default]);
  if (!ShaderCompiler::LinkProgram(shaderProgram[ShaderProgram::Billboarding]))
  {
    cleanUp();
    return false;
  }
  uniformBlockBinding(shaderProgram[ShaderProgram::Billboarding]);
  uniformBlockBinding(shaderProgram[ShaderProgram::Billboarding], "InstanceBuffer", 1);
